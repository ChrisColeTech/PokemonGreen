...
exports.CUSTOM_ANIMATIONS = exports.ANIMATION_ROWS_LAYOUT = void 0;
exports.customAnimationBase = customAnimationBase;
exports.customAnimationArea = customAnimationArea;
const constants_1 = require("./constants");
exports.ANIMATION_ROWS_LAYOUT = {
    "thrust-n": 3,
    "thrust-w": 4,
    "thrust-s": 5,
    "thrust-e": 6,
...
function customAnimationBase(definition) {
    const firstFrame = definition.frames.at(0)?.at(0);
    if (!firstFrame) {
        throw new Error("Custom animation definition has no frames.");
    }
    const rowName = firstFrame.split(",").at(0) ?? "";
    const baseAnimation = rowName.split("-").at(0) ?? "";
    if (baseAnimation.length === 0) {
        throw new Error(`Unable to determine base animation from frame '${firstFrame}'.`);
    }
    return baseAnimation;
}
function customAnimationArea(name, definition, yOffset) {
    const rows = definition.frames.length;
    const columns = definition.frames.at(0)?.length ?? 0;
    if (rows === 0 || columns === 0) {
        throw new Error(`Custom animation '${name}' has an empty frame grid.`);
    }
    for (const [rowIndex, row] of definition.frames.entries()) {
        if (row.length !== columns) {
            throw new Error(`Custom animation '${name}' row ${rowIndex} has ${row.length} columns, expected ${columns}.`);
        }
    }
    if (definition.frameSize < constants_1.FRAME_SIZE) {
        throw new Error(`Custom animation '${name}' frameSize ${definition.frameSize} is smaller than standard frame size ${constants_1.FRAME_SIZE}.`);
    }
    return {
        name,
        baseAnimation: customAnimationBase(definition),
        frameSize: definition.frameSize,
        rows,
        columns,
        width: definition.frameSize * columns,
        height: definition.frameSize * rows,
        yOffset
    };
}
