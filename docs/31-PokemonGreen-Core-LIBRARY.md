# 31-PokemonGreen-Core-LIBRARY

Reference documentation for PokemonGreen.Core - the shared game logic library powering both 2D and 3D Pokemon game experiences.
...
- Pokemon data (species, stats, moves, growth rates)
...
| `AllyModel` | `SkeletalModelData?` | Player's Pokemon 3D model |
| `FoeModel` | `SkeletalModelData?` | Enemy Pokemon 3D model |
...
| `EnterBattle` | `(BattleBackground)` | Start battle with test Pokemon |
| `EnterBattle` | `(BattlePokemon ally, BattlePokemon foe, BattleBackground)` | Start battle with specific Pokemon |
...
| Method | Signature | Purpose |
|--------|-----------|---------|
| `Reset` | `(float foeHeight, float allyHeight)` | Reset to start position, adjusted for Pokemon sizes |
...
Renders 3D battle backgrounds, platforms, and Pokemon models.
...
| `AllyModel` | `SkeletalModelData?` | Player Pokemon model |
| `FoeModel` | `SkeletalModelData?` | Enemy Pokemon model |
...
| `SetPokemonModels` | `(SkeletalModelData? ally, SkeletalModelData? foe)` | Set Pokemon models |
| `ClearPokemonModels` | `()` | Remove Pokemon models |
...
Wrapper for a Pokemon in battle with animated HP display.
...
| `Source` | `PartyPokemon?` | Original party Pokemon |
...
| `FromParty` | `(PartyPokemon)` → `BattlePokemon` (static) | Create from party Pokemon |
...
| Method | Signature | Purpose |
|--------|-----------|---------|
| `SetAlly` | `(BattlePokemon)` | Replace ally (for switch-in) |
...
| `OpenFightMenu` | `(BattlePokemon?, Action closeFightMenu)` | Show move selection |
...
Pokemon in player's party with full stat tracking.
...
| `Create` | `(int speciesId, int level, Gender, Random?)` → `PartyPokemon` (static) | Create new Pokemon |
