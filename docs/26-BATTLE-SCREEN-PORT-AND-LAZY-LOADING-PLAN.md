...
| File | Lines | Role |
|------|-------|------|
| `src/PokemonGreen.Core/Battle/BattleScreen3D.cs` | 802 | Full battle screen: 3D scene + UI + state machine |
| `src/PokemonGreen.3D/Game1.cs` | 691 | 3D POC shell: overworld + delegates to BattleScreen3D |
| `src/PokemonGreen/Game1.cs` | 1312 | 2D game: original battle code (reference) |
| `src/PokemonGreen.Core/Encounters/EncounterRegistry.cs` | 186 | Map encounter loading + weighted random selection |
| `src/PokemonGreen.Core/Pokemon/SpeciesRegistry.cs` | ~120 | Species data: name, stats, ModelFolder |
| `src/PokemonGreen.Assets/BattleModelLoader.cs` | 302 | DAE model loading for battle backgrounds |
| `src/PokemonGreen.Assets/SkeletalModelData.cs` | ~300 | Skinned Pokemon model with animation |
| `src/PokemonGreen.Assets/Data/species.json` | 12031 | 802 species definitions |
| `src/PokemonGreen.BgEditor/` | — | Electron map/animation editor |
