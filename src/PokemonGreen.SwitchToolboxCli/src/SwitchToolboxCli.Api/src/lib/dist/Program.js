...
export async function main() {
    ...
    if (allMode) {
        ...
        const allModels = Array.from(loader.FindFilesByExtension('.trmdl'));
...
async function exportModel(loader, modelPath, outputDir) {
    fs.mkdirSync(outputDir, { recursive: true });
    ...
    try {
        ...
        const mdl = FlatBufferConverter.DeserializeFrom(trmdlBytes);
        const modelDir = getDirectoryOrEmpty(normalizedModel);
        const pending = [];
        const extracted = new Set([normalizedModel.toLowerCase()]);
        // Enqueue mesh files
        if (mdl.Meshes) {
        ...
        while (pending.length > 0) {
            ...
            if (ext === '.trmsh') {
                // TRMSH → buffer file
                try {
                    const msh = FlatBufferConverter.DeserializeFrom(bytes);
                ...
                try {
                    const mtr = FlatBufferConverter.DeserializeFrom(bytes);
                    if (mtr?.Materials) {
        ...
        let texCount = 0;
        function* enumerateBntxFiles(dir) {
        ...
        for (const bntxFile of enumerateBntxFiles(tempRoot)) {
            try {
                ...
                const textures = BntxDecoder.decode(bntxBytes);
        ...
        if (exportData.Armature) {
            const animFiles = Array.from(loader.FindFiles((name) => name.toLowerCase().startsWith(modelDataDir.toLowerCase()) &&
            ...
            for (const [hash, animName] of animFiles) {
                try {
                    ...
                    if (animBytes === null)
                        continue;
                    const animFb = FlatBufferConverter.DeserializeFrom(animBytes);
        ...
        if (animCount > 0 && fs.existsSync(texOutDir)) {
            ...
            for (const texFile of texFiles) {
                fs.copyFileSync(path.join(texOutDir, texFile), path.join(animTexDir, texFile));
            }
...
// ---------- Helpers ----------
function writeExtractedFile(root, relPath, data) {
