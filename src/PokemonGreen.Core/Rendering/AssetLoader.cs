...
public static Stream? GetSpriteStream(string name)
{
    ...
    var resourceName = $"PokemonGreen.Core.Sprites.{name}.png";
...
{
    var assembly = typeof(AssetLoader).Assembly;
    var resourceName = $"PokemonGreen.Core.Items.{name}.png";
    return assembly.GetManifestResourceStream(resourceName);
}
...
{
    var assembly = typeof(AssetLoader).Assembly;
    var resourceName = $"PokemonGreen.Core.Player.{name}.png";
    return assembly.GetManifestResourceStream(resourceName);
}
...
{
    var assembly = typeof(AssetLoader).Assembly;
    var resourceName = $"PokemonGreen.Core.NPCs.{name}.png";
    return assembly.GetManifestResourceStream(resourceName);
}
...
{
    var assembly = typeof(AssetLoader).Assembly;
    var resourceName = $"PokemonGreen.Core.NPCs.{name}.json";
...
{
    var assembly = typeof(AssetLoader).Assembly;
    var resourceName = $"PokemonGreen.Core.Data.{name}.json";
    using var stream = assembly.GetManifestResourceStream(resourceName);
    if (stream == null)
        return null;
    using var reader = new StreamReader(stream);
