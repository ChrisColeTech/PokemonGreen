...
{
    private static readonly Dictionary<string, BattleModelData> _cache = new();

    /// <summary>
    /// Load a Pokemon model by its folder name (e.g. "pm0001_00").
    /// Returns null if the model folder or model.dae doesn't exist.
    /// </summary>
    public static BattleModelData? Load(string modelFolder, GraphicsDevice graphicsDevice)
    {
        ...
        var model = BattleModelLoader.Load(modelPath, graphicsDevice);
