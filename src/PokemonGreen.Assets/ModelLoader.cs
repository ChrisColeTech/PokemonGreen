...
/// <summary>
/// Loads 3D models on demand from the Pokemon3D/ asset directory.
/// Each model has a folder (e.g. Pokemon3D/pm0001_00/) containing:
...
public static class ModelLoader
{
    ...
    public static void InitializeDevPaths()
    {
        ...
        string assemblyDir = Path.GetDirectoryName(typeof(ModelLoader).Assembly.Location) ?? "";
    ...
    /// <summary>
    /// Load a model by its folder name (e.g. "pm0001_00" or "characters/trainers/tr0001_00").
    /// Returns null if the model folder doesn't exist or contains no .dae model file.
    /// Resolves the model filename from manifest.json (modelFile field) or falls back
    /// to "model.dae". Automatically loads animation clips from manifest if present.
    ...
    public static SkeletalModelData? Load(string modelFolder, GraphicsDevice graphicsDevice)
    {
        ...
        string? manifestText = File.Exists(manifestPath) ? File.ReadAllText(manifestPath) : null;

        // Resolve model filename: check manifest modelFile first, fall back to model.dae
        string modelPath = ResolveModelPath(basePath, manifestText);
        ...
        if (manifestText != null)
        {
            try
            {
                LoadClipsFromManifest(model, basePath, manifestText);
            }
            catch (Exception ex)
            {
    ...
    /// <summary>
    /// Resolve the .dae model file path. Checks manifest.json for modelFile field
    /// (from models[0].modelFile), falls back to "model.dae".
    /// </summary>
    private static string? ResolveModelPath(string basePath, string? manifestText)
    {
        if (manifestText != null)
        {
            try
            {
                ...
                if (TryGetProp(root, "models", out var modelsArray))
                {
                    ...
                    {
                        var file = TryGetString(modelEntry, "modelFile");
                        if (file != null)
        ...
        return Path.Combine(basePath, "model.dae");
    }
    ...
    /// </summary>
    private static void LoadClipsFromManifest(SkeletalModelData model, string basePath, string manifestText)
    {
        using var doc = JsonDocument.Parse(manifestText);
        var root = doc.RootElement;
