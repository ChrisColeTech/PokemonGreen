...
// ---------------------------------------------------------------------------
// Manifest types (matches doc section 3.3)
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  for (let gi = 0; gi < groups.length; gi++) {
    ...
    const firstName = modelGroup.model.find(m => m.mesh.length > 0)?.name ?? '';
    const folderName = `${String(gi).padStart(4, '0')}_${sanitizeName(firstName) || `entry_${String(startEntry).padStart(5, '0')}`}`;
    ...
    if (splitModelAnims) {
      ...
      for (let mi = 0; mi < modelGroup.model.length; mi++) {
        ...
        for (let ci = 0; ci < skeletalClips.length; ci++) {
          ...
          clipEntries.push({
            index: ci,
            name: clip.name || `clip_${String(ci).padStart(3, '0')}`,
            file: clipRelPath,
